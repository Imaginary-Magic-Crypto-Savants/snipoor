import type { LicenseState, LicenseStatus, RedeemResponse, RefreshResponse, EntitlementsResponse, EntitlementsResult } from "./types"
...
// Live read of the current wallet-mint allowance without touching the refresh
// token -- safe to call as often as needed (popup open, before a mint run, a
// manual "sync" button) since nothing rotates and a failure never clears the
// license. The only path allowed to touch the refresh token here is a genuine
// 401 (access token actually expired), which falls through to the same
// rotate-safe refreshLicense() resolveStatus() already uses.
export async function fetchEntitlements(): Promise<EntitlementsResult> {
  if (gateDisabled()) return { walletAllowance: null, stale: false }

  const state = await loadLicense()
  if (!state) return { walletAllowance: null, stale: true }

  let res: Response
  try {
    res = await fetch(apiUrl("/auth/entitlements"), {
      headers: { authorization: `Bearer ${state.accessToken}` },
    })
  } catch {
    return { walletAllowance: state.walletAllowance ?? null, stale: true }
  }

  if (res.status === 401) {
    const refreshed = await refreshLicense(state)
    return { walletAllowance: refreshed.walletAllowance ?? null, stale: !refreshed.licensed }
  }
  if (!res.ok) {
    return { walletAllowance: state.walletAllowance ?? null, stale: true }
  }
  ...
  await saveLicense({ ...state, walletAllowance: data.walletAllowance ?? null })
  return { walletAllowance: data.walletAllowance ?? null, stale: data.stale }
}
