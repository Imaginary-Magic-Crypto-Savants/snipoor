...
export type Message =
  ...
  | { type: "CONSOLIDATE"; payload: { chainId: number; speed: SpeedTier; tokenAddress?: string; destination?: string } }
  ...
  | { type: "CONSOLIDATE_NFTS"; payload: { chainId: number; contractAddress: string; destination: string; speed: SpeedTier; amountPerWallet?: number } }
  | { type: "GET_ACTIVITY" }
  ...
  | { type: "GET_PANEL_MODE" }
  | { type: "GET_LAST_MINT" }
  | { type: "SCATTER_ELIGIBLE"; payload: { chainId: number; contractAddress: string; slug: string; walletAddresses: string[] } }
  | { type: "SCATTER_MINT"; payload: { chainId: number; contractAddress: string; slug: string; listId: string; quantity: number; speed: SpeedTier; walletAddresses: string[]; gasLimit?: number } }
  | { type: "LICENSE_STATUS" }
  | { type: "LICENSE_REDEEM"; payload: { code: string } }
  | { type: "LICENSE_REFRESH" }
  | { type: "LICENSE_DEACTIVATE" }
  | { type: "LICENSE_ENTITLEMENTS" }
...
export type EntitlementsData = import("~/lib/license/types").EntitlementsResult
