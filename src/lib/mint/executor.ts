...
export async function executeMint(
  ...
  const perWallet = config.argsPerWallet
  const hasCallerPlaceholder = config.args.some((a) => a === "{{CALLER}}")
  // A shared calldata is only reusable when there's no per-wallet override and
  // no caller placeholder; otherwise each wallet encodes its own.
  const sharedCalldata = perWallet || hasCallerPlaceholder ? null : iface.encodeFunctionData(config.functionName, config.args)
  ...
  for (let i = 0; i < walletKeys.length; i += batchSize) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (wk) => {
        try {
          const walletArgs = perWallet?.[wk.address.toLowerCase()] ?? resolveArgs(config.args, wk.address)
          const calldata = sharedCalldata ?? iface.encodeFunctionData(config.functionName, walletArgs)
