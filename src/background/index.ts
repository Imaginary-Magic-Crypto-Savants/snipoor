...
async function handleMessage(msg: Message): Promise<Response> {
  switch (msg.type) {
    ...
    case "CONSOLIDATE": {
      ...
      // Destination defaults to master but can be ANY wallet. It must be one of
      // our own wallets — consolidate is a sweep, not a send, and a typo'd
      // external address here would drain every wallet irrecoverably.
      const destination = msg.payload.destination?.toLowerCase() || masterAddr.toLowerCase()
      const ownsDest =
        activeMaster.wallets.some((w) => w.address.toLowerCase() === destination) ||
        vault.importedWallets.some((w) => w.address.toLowerCase() === destination)
      if (!ownsDest) return { ok: false, error: "Destination must be one of your wallets" }
      const destAddr =
        activeMaster.wallets.find((w) => w.address.toLowerCase() === destination)?.address ??
        vault.importedWallets.find((w) => w.address.toLowerCase() === destination)!.address

      // Sources: every enabled wallet except the destination. When the
      // destination is a sub, the master itself gets swept too.
      const subKeys = activeMaster.wallets
        .filter((w) => w.address.toLowerCase() !== destination && !vault.disabledAddresses.has(w.address.toLowerCase()))
      ...
      try {
        ...
        const results = await consolidateETH(msg.payload.chainId, allKeys, destAddr, msg.payload.speed, onConsolidateProgress)
        for (const r of results) {
          await addActivity({
            id: crypto.randomUUID(),
            timestamp: Date.now(),
    ...
    case "SCATTER_ELIGIBLE": {
      try {
        const { fetchAggregatedEligibility } = await import("~/lib/mint/scatter")
        const lists = await fetchAggregatedEligibility(
          msg.payload.slug,
          msg.payload.contractAddress,
          msg.payload.walletAddresses
        )
    ...
    case "SCATTER_MINT": {
      // Resolve each wallet's mint (proof from Scatter, validated + re-encoded
      // here), then mint through the same hardened executor as EXECUTE_MINT.
      const { fetchEligibleLists, resolveListMint, ARCHETYPE_MINT_ABI_JSON } = await import("~/lib/mint/scatter")
      try {
        const lists = await fetchEligibleLists(msg.payload.slug, msg.payload.contractAddress)
        const list = lists.find((l) => l.id === msg.payload.listId)
        if (!list) return { ok: false, error: "That invite list is no longer available." }
        ...
        const { resolved, skipped } = await resolveListMint(
          msg.payload.chainId,
          msg.payload.contractAddress,
          list,
          msg.payload.quantity,
          walletKeys.map((k) => k.address)
        )
        if (resolved.length === 0) {
          const reason = skipped[0]?.reason ?? "No wallets were eligible for this list."
          return { ok: false, error: `Nothing to mint: ${reason}` }
        }

        const argsPerWallet: Record<string, unknown[]> = {}
        for (const r of resolved) argsPerWallet[r.address.toLowerCase()] = r.args
        const mintKeys = walletKeys.filter((k) => argsPerWallet[k.address.toLowerCase()])
        const value = formatEther(resolved[0].valueWei)
        ...
        const results = await executeMint(
          {
            ...
            value,
            speed: msg.payload.speed,
            walletAddresses: mintKeys.map((k) => k.address),
            gasLimit: msg.payload.gasLimit,
          },
          mintKeys,
          ARCHETYPE_MINT_ABI_JSON,
          (current, total) => broadcastToPopup({ type: "TX_PROGRESS", current, total, label: "Minting..." })
        )
        for (const r of results) {
          await addActivity({
        ...
        return { ok: true, data: { succeeded, failed, total: results.length, results, skipped } }
      } catch (e) {
        return { ok: false, error: cleanError(e) }
      }
    }
    ...
    case "CONSOLIDATE_NFTS": {
      ...
      try {
        ...
        for (const bal of balances) {
          ...
          if (pk) {
            // Optional global per-wallet cap: "send N from every wallet that
            // holds this NFT". Undefined/0 = send all.
            const cap = msg.payload.amountPerWallet
            const tokenIds = cap && cap > 0 ? bal.tokenIds.slice(0, cap) : bal.tokenIds
            transfers.push({ fromAddress: bal.address, fromPrivateKey: pk, tokenIds })
          }
        }
