...
import { sendMessage, type WalletsData, type ExportData, type ApiKeysData } from "~/lib/messaging"
...
export function WalletsPanel({ wallets, ethPriceUsd, balanceLoading, balanceFetchedAt, onCopy, onRefresh, onRefreshBalances }: WalletsPanelProps) {
  ...
  const handleConsolidate = async () => {
    ...
    const res = await sendMessage<{ succeeded: number; failed: number }>({
      type: "CONSOLIDATE",
      payload: { chainId: operationChain, speed: fundSpeed },
    })
    if (res.ok && res.data) {
      if (res.data.succeeded === 0 && res.data.failed === 0) {
        setResult("No funds to consolidate — all ETH in master wallet")
  ...
  const handleConsolidateNFTs = async () => {
    ...
    const res = await sendMessage<{ results: NFTTransferResult[] }>({
      ...
      payload: { chainId: operationChain, contractAddress: nftContract, destination: nftDest, speed: nftSpeed },
    })
    if (res.ok && res.data) {
        ...
        {nftBalances.length > 0 && (
          <>
            ...
            {nonMasterNFTs > 0 && (
              ...
              >
                CONSOLIDATE {nonMasterNFTs} NFTs TO MASTER
              </button>
            )}
          </>
        )}
        ...
        {nftInfo && (
          <div style={{ fontSize: 10, color: "var(--text2)", marginBottom: 8, letterSpacing: 1 }}>
            ...
            {totalNFTs - transferCount > 0 && (
              <span style={{ color: "var(--text3)" }}> ({totalNFTs - transferCount} already at dest)</span>
            )}
          </div>
        )}
        <div className="input-group">
  ...
  if (view === "consolidate") {
    ...
    return (
      <div className="panel-body">
        ...
        <div className="step-desc" style={{ marginBottom: 8 }}>
          Consolidate ETH from {subCount} wallet{subCount !== 1 ? "s" : ""} to master.
        </div>
        <div className="input-group">
          <label className="input-label">Speed</label>
          <div className="speed-selector">
  ...
  if (view === "manage-menu") {
    return (
      <div className="panel-body">
        ...
        <div className="step-desc" style={{ marginBottom: 12 }}>
          Disperse ETH to sub-wallets or consolidate back to master.
        </div>
